...
const updateStaticReleaseMetaData = async (
  app: NucleusApp,
  ...
  await positioner.withLock(app, async lock => {
    await positioner.updateDarwinReleasesFiles(
      lock,
      app,
      upToDateChannel,
      'x64'
    );
    await positioner.updateWin32ReleasesFiles(
      lock,
      app,
      upToDateChannel,
      'ia32'
...
const hasPermission = (req: Express.Request, app: NucleusApp) => {
  return (
    req.user &&
    ((req.user as User).isAdmin || app.team.indexOf(req.user.id) !== -1)
  );
};
    ...
    if (checkField(req, res, 'name')) {
      ...
      if (MAGIC_NAMES.includes(req.body.name)) {
        return res.status(400).json({
          error: `You can not call your application ${req.body.name}`
        });
      }
      ...
      if (req.body.name === '') {
        return res
          .status(400)
          .json({ error: 'Your app name can not be an empty string' });
      }
      ...
      if (req.file) {
        ...
        res.json(
          await driver.createApp(req.user as User, req.body.name, iconBuffer)
        );
      } else {
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['team'])) {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ error: 'Provided parameter "team" is not valid JSON' });
      }
      if (
        Array.isArray(team) &&
        team.length > 0 &&
        team.indexOf(req.user.id) !== -1
      ) {
        d(
          `Updating team for app: '${req.targetApp.name}' to be: [${team.join(
            ', '
          )}]`
        );
        ...
        d(
          `Invalid team array for app '${req.targetApp.slug}' was sent to Nucleus`
        );
...
router.post(
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['url', 'secret'])) {
      ...
      d(
        `Creating new WebHook: '${req.body.url}' for app: '${req.targetApp.slug}'`
      );
      const rawHook = await driver.createWebHook(
        req.targetApp,
        req.body.url,
        req.body.secret
      );
...
router.delete(
  ...
  a(async (req, res) => {
    if (stopNoPerms(req, res)) return;
    d(
      `Deleting WebHook: '${req.params.webHookId}' for app: '${req.targetApp.slug}'`
    );
    const rawHook = await driver.getWebHook(
      req.targetApp,
      Number(req.params.webHookId)
    );
...
router.post(
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['name'])) {
      d(
        `Creating new channel: '${req.body.name}' for app: '${req.targetApp.slug}'`
      );
...
router.get(
  ...
  a(async (req, res) => {
    if (stopNoPerms(req, res)) return;
    const channel = await driver.getChannel(
      req.targetApp,
    ...
    d(
      `Fetching temporary releases for app: ${req.targetApp.slug} and channel: ${channel.name}`
    );
...
router.get(
  ...
  a(async (req, res) => {
    ...
    const save = await driver.getTemporarySave(
      Number(req.params.temporarySaveId)
    );
    if (!save) {
      return res.status(404).json({
  ...
  a(async (req, res) => {
    ...
    if (
      !(await positioner.withLock(req.targetApp, async lock => {
        ...
        d(
          `Tested files: [${save.filenames.join(
            ', '
          )}] but stored: [${storedFileNames.join(', ')}]`
        );

        // Get up to date channel
        const upToDateChannel = (await driver.getChannel(
          req.targetApp,
          req.params.channelId
        ))!;
        const storedVersion = upToDateChannel.versions.find(
          v => v.name === save.version
        )!;
        const storedFiles = storedVersion.files.filter(f =>
          storedFileNames.includes(f.fileName)
        );
        ...
        for (const file of storedFiles) {
          ...
          if (upToDateFile) {
            ...
            d(
              'Database inconsistency detected while releasing for file:',
              file.id
            );
          }
        }
        await positioner.cleanUpTemporaryFile(
          lock,
          req.targetApp,
          save.saveString
        );
    ...
    const version = updatedChannel.versions.find(
      version => version.name === save.version
    );
    if (!version) return;
    if (!versionExists)
      runHooks(req.targetApp, hook => hook.newVersion(updatedChannel, version));
    if (storedFileNames!.length > 0) {
      runHooks(req.targetApp, hook =>
        hook.newVersionFile(updatedChannel, version)
      );
    }
  })
);
...
router.post(
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['version', 'dead'])) {
      const internalVersion = channel.versions.find(
        v => v.name === req.body.version
      );
      if (!internalVersion) {
      ...
      if (isGreatest) {
        ...
        return res
          .status(400)
          .json({ error: "You can't kill the latest version" });
      }
      d(
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['version', 'rollout'])) {
      ...
      if (version.rollout === 100) {
        return res.status(400).json({
          error:
            'You cannot change the rollout of a version once it has reached 100%'
        });
      }
      d(
      ...
      const updatedVersion = updatedChannel.versions.find(
        v => v.name === req.body.version
      );
      if (updatedVersion) {
        await positioner.withLock(req.targetApp, async lock => {
    ...
    if (checkFields(req, res, ['version', 'platform', 'arch'])) {
      ...
      if (['darwin', 'linux', 'win32'].indexOf(req.body.platform) !== -1) {
        ...
        if (new Set(fileNames).size !== fileNames.length) {
          return res.status(400).json({
            error:
              'Looks like you tried to upload two or more files with the same file name'
          });
        }
